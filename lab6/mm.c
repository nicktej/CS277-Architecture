 ...
 */
void read_matrix_row_order(FILE *stream, int n, double *m[n])
{
    double f;
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
...
/**
 * Read a square matrix from a FILE stream in col-major order.
 *
 * @param stream  input from which to read the square matrix
 * @param n       dimension of the square matrix
 * @param m       allocated col-major n*n matrix
 */
void read_matrix_col_order(FILE *stream, int n, double *m[n])
{
    double f;
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
...
int main(int argc, char *argv[])
{
    ...
    read_matrix_row_order(stdin, n, a);
    read_matrix_col_order(stdin, n, b);
